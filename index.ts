...
function setupCurvedWorld(app) {
  ...
  CurvedWorld.prototype.initialize = function () {
    ...
    this.activeCamera = this.app.root.findByName('Camera') || this.app.root.findByType('camera')[0];
    ...
    console.log('CurvedWorld script initialized with camera:', this.activeCamera);
...
CurvedWorld.prototype.updateShader = function (material) {
  ...
  curvedEntity.script.create('curvedWorld', {
    attributes: {
      curvePower: 1 // Set the initial curve power
    }
  });
}
